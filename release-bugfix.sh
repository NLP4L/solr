...
echo -n "Have you executed mvn test and has it successfully done? (y/n) "
read ANS
if [ $ANS != "y" ]; then
  exit 1
fi
...
rm CHANGES.txt.temp
sed -e 1,10s:\<version\>$THIS_REL_NUM\<\/version\>:\<version\>$NEXT_REL_NUM\<\/version\>: pom.xml > pom.xml.temp
...
echo "2. build nlp4l-solr-${THIS_REL_NUM}.jar file by executing mvn package. And zip that"
echo "3. Go to https://github.com/NLP4L/solr/releases/tag/rel-${THIS_REL_NUM}"
echo "4. click [Edit tag] button and drop down the zip file to the drop down box and click [Publish release]"
echo "5. to publish the artifact to the maven repository thru Sonatype, execute (1) sbt publishSigned and then (2) sbt sonatypeRelease"
echo "6. to go back to master, git checkout master"
echo -e "\\nThe download link will be  https://github.com/NLP4L/solr/releases/download/rel-${THIS_REL_NUM}/nlp4l-solr-${THIS_REL_NUM}.zip"
